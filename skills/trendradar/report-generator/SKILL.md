...
## v1.1 部署注意（2026-06-09 实战发现）

- **cron prompt 必须在开头显式 export 4 个环境变量**：`TRENDRADAR_HOME` / `HERMES_HOME` / `PYTHON` / `PYTHONPATH=$TRENDRADAR_HOME`。scheduler **不会**自动注入（验证于 2026-06-09）—— 没设就 `ModuleNotFoundError: trendradar`。
- **`enabled_toolsets` 必须显式包含 `"delegation"`**——`hermes cron create / edit` 都不暴露 `--toolset` 选项，直接改 `~/.hermes/cron/jobs.json` 把 `null` 改成 `["default", "delegation"]`，改完 `hermes gateway restart`。
- **周报 prompt 已含 `delegate_task`（信息茧房突围 cross-domain）**，月报 prompt 走 `deep-research-cli`（同样需要 delegation）。如果只设 `default` toolset，agent 跑出来会报告"工具不可用"然后跳过整个深度搜索部分。
- 详细 Windows path 陷阱、scheduler scripts_dir、`PYTHONPATH=.parent` bug 等见 `system-config` skill。
