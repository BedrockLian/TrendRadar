...
def generate_cron_prompt(steps: dict) -> str:
    ...
    lines.append('- `status: "ok"` → output `briefing` field piped through validate_output.py (auto-delivery)')
    lines.append('')
    lines.append('### Output validation')
    lines.append('')
    lines.append('Pipe briefing through validate_output.py to catch format violations:')
    lines.append('')
    lines.append('```bash')
    lines.append('BRIEFING=$(echo "$RESULT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get(\\\"briefing\\\",\\\"\\\"))" 2>/dev/null)')
    lines.append('VALIDATED=$(echo "$BRIEFING" | $PYTHON scripts/validate_output.py --push-id {PUSH_ID} 2>/dev/null)')
    lines.append('echo "$VALIDATED"')
    lines.append('```')
    lines.append('')
    lines.append('If validation fails and falls back to archive, a warning goes to stderr (harmless).')
    lines.append('The stdout is always the deliverable content.')
    lines.append("")
