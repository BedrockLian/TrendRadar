...
所有常量定义已迁移到子模块：
  ...
  runtime/file_utils.py   — 路径工厂/压缩 I/O
  runtime/logging_config.py — Logger 工厂

业务模块从这里读取聚合后的配置常量和运行时服务。
"""
...
# ── 文件工具 ─────────────────────────────────────────────
from trendradar.runtime.file_utils import (
...
# ── 日志工厂 ─────────────────────────────────────────────
from trendradar.runtime.logging_config import get_logger  # noqa: E402, F401
...
def get_storage():
    ...
    with _STORAGE_LOCK:
        if _STORAGE_VAL is not None:
            return _STORAGE_VAL
        from trendradar.runtime.storage import Storage
