...
from trendradar.runtime.common import CST, Lazy

from trendradar.runtime.settings import get_data_dir, get_cache_dir, get_config_dir, write_compressed
from trendradar.runtime.settings import EXTERNAL_CONCURRENT, TIMEOUT_SEC
from trendradar.runtime.settings import PROXY_URL, needs_proxy, check_proxy_alive
...
_CONFIG = Lazy(lambda: json.loads((get_config_dir() / 'sources.json').read_text(encoding='utf-8')))
...
def fetch_all(push_id: str = '') -> dict:
    ...
    print(
        f'[FETCH] {len(sources)}源（直连 {len(direct_sources)} + 代理 {len(proxy_sources)}）',
        file=sys.stderr,
    )
    ...
    try:
        ...
        for item in merged:
            if (fp := ht.make_fingerprint(item.get('title', ''), item.get('url', ''))) in hi:
                item['_heat'] = hi[fp]
        print(
            f'[HEAT] 新增{stats["new"]}条 更新{stats["updated"]}条 共{stats["total_active"]}活跃',
            file=sys.stderr,
        )
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print(f'[{start:%H:%M:%S}] 开始多线程抓取...', file=sys.stderr)
    ...
    print(
        f'[{datetime.now(CST):%H:%M:%S}] 完成: {len(items)}条 '
