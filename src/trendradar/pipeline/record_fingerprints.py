...
from trendradar.intelligence.heat_tracker import make_fingerprint

from trendradar.runtime.settings import get_data_dir, get_storage, DOMAINS
...
def record(push_id: str):
    ...
    for p in paths:
        if p.exists():
            try:
                curated = json.loads(p.read_text(encoding='utf-8'))
                break
