...
#!/usr/bin/env python3
from trendradar.runtime.settings import get_logger
...
from trendradar.runtime.settings import get_data_dir, get_cache_dir, DOMAINS
DATA_DIR = get_data_dir()
CACHE_DIR = get_cache_dir()

from trendradar.runtime.common import gen_run_id, run_id_marker, set_run_id_ctx
from trendradar.runtime.settings import write_compressed

# Sprint 3 perf: raw JSON 进程内共享缓存 (pipeline_orchestrator 和 push_prepare 各读一次 → 共享)
from trendradar.runtime.lazy import Lazy as _Lazy
...
def get_raw_today(force_reload: bool = False) -> dict:
    ...
    if _raw_today_cache is None or force_reload:
        ...
        if raw_path.exists():
            try:
                _raw_today_cache = json.loads(raw_path.read_text(encoding='utf-8'))
            except Exception:
...
def ensure_raw_exists(push_id: str, skip_fetch: bool = False):
    ...
    from trendradar.sources.fetch_feeds import fetch_all
    ...
    try:
        from trendradar.runtime.file_utils import _get_zstd
        if _get_zstd():
...
def run_curation(push_id: str, skip_fetch: bool = False) -> dict:
    ...
    """
    import trendradar.pipeline.curate_and_push as curate
    ...
    try:
        raw = json.loads((CACHE_DIR / f'raw_{today}.json').read_text(encoding='utf-8')).get('items', [])
...
def get_today_fingerprints() -> list:
    ...
    import sqlite3
    try:
        import trendradar.intelligence.heat_tracker as ht
        conn = ht.get_db()
    except Exception:
...
if __name__ == '__main__':
    ...
    import sys
    from trendradar.runtime.common import EXIT_CONFIG_ERROR, EXIT_FATAL
