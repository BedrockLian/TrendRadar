...
#!/usr/bin/env python3
"""
render_deep_analysis.py — 格式化 flash 深度分析为统一 Markdown。
...
from trendradar.runtime.paths import get_data_dir
...
def clean(text: str) -> str:
    """Normalize markdown while keeping the analysis structure.

    Paired **...** markers are kept because Codex renders standard Markdown.
    flash sub-agent 常写出未闭合的 **（末尾多写当加粗没闭合），
    clean() 不剥，反而就地补齐成对 —— LLM 意图就是想加粗，
    让标准 Markdown 渲染时真加粗。策略：
      1) 先保护已配对的 **X**（X 内不出现 **）
...
def format_analysis(text: str, topic: str = "深度分析",
    ...
    """Format analysis text for the Codex briefing output.
    ...
    for para in paragraphs:
        ...
        for line in lines:
            ...
            # Keep intentional leading bold markers in standard Markdown.
