...
输出: Markdown 简报到 stdout，供 Codex 直接读取
"""
...
from trendradar.runtime.settings import get_data_dir, DOMAINS, DOMAIN_LABELS, SLOT_NAMES
...
def _get_item_priority(source_platform: str) -> int:
    ...
    """
    from trendradar.sources.fetch_feeds import _load_config
    try:
...
def _format_item(idx, item, push_id):
    ...
    """
    title = _shorten(item.get('title_cn') or item.get('title') or '', 100)
    ...
    if priority < 2:  # P0/P1: include summary
        summary = _shorten(item.get('summary_cn') or item.get('summary') or '', 120 if priority == 0 else 100)
        if summary:
...
def render_briefing(push_id: str) -> str:
    ...
    header = f"### TrendRadar 日报 · {today_display}（{slot_name}）\n\n\n📋 **共 {total} 条** · {counts_str}"
    ...
    footer = f"\n📌 *共{total}条 · 自动生成于{today_display} · TrendRadar*"

    # Assemble: header + 2 blank lines + sections (each separated by 2 blank lines) + 2 blank lines + footer
    output = header + '\n\n\n' + '\n\n\n'.join(sections) + '\n\n\n' + footer
    # Archive the canonical Markdown for audit and report aggregation.
