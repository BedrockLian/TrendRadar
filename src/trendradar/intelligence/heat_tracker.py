...
from trendradar.runtime.settings import get_data_dir, get_storage
...
def _init_db_impl():
    ...
    from trendradar.runtime.settings import ensure_db_migrated
    ...
    with _FINGERPRINT_LOCK:
        ...
        if url:
            try:
                ...
                parsed = urlparse(url)
                from trendradar.runtime.settings import FINGERPRINT_URL_SEGMENTS
        ...
        from trendradar.runtime.settings import FINGERPRINT_MD5_LEN
...
def _gen_fingerprints(items: list, push_id: str, now: str) -> dict:
    ...
    fp_map = {}
    from trendradar.runtime.settings import HEAT_WORDS
    for item in items:
    ...
    try:
        ...
        if insert_batch:
            conn.executemany("""
                ...
                VALUES (?, ?, ?, ?, 1, 1, ?, ?, ?, ?, ?, 'active')
            """, insert_batch)
        from trendradar.runtime.settings import HEAT_SLEEP_HOURS
...
def _calc_heat(fp: str, title: str, row, now: datetime) -> dict:
    ...
    if row:
        ...
        from trendradar.runtime.settings import HEAT_DEEP_CYCLES, HEAT_DEEP_SPAN, HEAT_SUSTAINED_CYCLES, HEAT_SUSTAINED_SPAN
