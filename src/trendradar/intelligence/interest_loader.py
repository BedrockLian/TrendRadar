...
from trendradar.runtime.settings import TRENDRADAR_HOME


_INTERESTS_LOCK = threading.Lock()
_INTERESTS_VAL: tuple = None
_INTERESTS_SENTINEL = object()
...
def load_interests() -> tuple[frozenset, frozenset]:
    ...
    回退支持旧版 .txt 格式。
    """
    global _INTERESTS_VAL
    if _INTERESTS_VAL is not None:
        return _INTERESTS_VAL
    with _INTERESTS_LOCK:
        ...
        if yaml_path.exists():
            ...
            _INTERESTS_VAL = (frozenset(), frozenset())
            return _INTERESTS_VAL
        ...
        _INTERESTS_VAL = (frozenset(positive), frozenset(negative))
        return _INTERESTS_VAL
