...
from trendradar.runtime.settings import get_data_dir, get_config_dir, get_logger

log = get_logger('ai-translate')

from trendradar.runtime.common import CST, find_curated_file, _parse_line_pairs
...
from trendradar.intelligence.llm_providers import (
    LLMProvider,
    LLMError,
    LLMAuthError,
    LLMRateLimit,
...
from trendradar.runtime.settings import TRANSLATE_BATCH_SIZE, TRANSLATE_BATCH_MAX_CONCURRENT
...
def _get_cache_path():
    ...
    if _CACHE_PATH is None:
        from trendradar.runtime.settings import get_cache_dir
...
def _load_and_scan(push_id: str) -> tuple[dict, list, list, Path]:
    ...
    from trendradar.runtime.settings import DOMAINS
    domains = DOMAINS
    items_to_translate = []
    items_to_expand = []
...
async def process_curated(push_id: str) -> dict:
    ...
    from trendradar.runtime.settings import get_api_key
    api_key = get_api_key()
