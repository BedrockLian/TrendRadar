...
SCRIPTS_DIR = str(Path(__file__).resolve().parents[1] / 'src' / 'trendradar' / 'scripts')
...
class TestBatchTranslateBoundary:
    ...
    def test_batch_size_exactly_5_single_batch(self):
        ...
        from trendradar.intelligence.ai_translate import batch_translate
        from trendradar.intelligence.ai_translate import _make_request as real_make_request
        ...
        async def _run():
            with patch('trendradar.intelligence.ai_translate._make_request', side_effect=tracked):
    ...
    def test_batch_size_exactly_5_sanity(self):
        ...
        async def _run():
            with patch('trendradar.intelligence.ai_translate._make_request', side_effect=mock):
                return await batch_translate(MagicMock(), items, 'fake_key', 'English')
...
class TestBatchTranslateAllBatching:
    ...
    def setup_method(self, method):
        ...
        from trendradar.intelligence.ai_translate import _get_cache_path
        p = _get_cache_path()
        if p.exists():
            p.unlink()
    ...
    def test_exactly_5_items_one_batch(self):
        ...
        from trendradar.intelligence.ai_translate import _batch_translate_all, BATCH_SIZE
        ...
        async def _run():
            with patch('trendradar.intelligence.ai_translate.batch_translate', side_effect=mock_batch_translate):
                with patch('trendradar.intelligence.ai_translate.circuit_broken', return_value=False):
                    session = MagicMock()
    ...
    def test_10_items_two_batches(self):
        ...
        from trendradar.intelligence.ai_translate import _batch_translate_all
...
class TestCircuitBreakerBoundary:
    ...
    def test_circuit_starts_closed(self):
        ...
        from trendradar.intelligence import ai_translate
    ...
    def test_circuit_breaker_skips_remaining_batches(self):
        ...
        from trendradar.intelligence.ai_translate import _batch_translate_all, reset_circuit
        from trendradar.intelligence import ai_translate
...
class TestSourceLangBoundary:
    ...
    def test_compound_platform_english(self):
        ...
        from trendradar.intelligence.ai_translate import get_source_lang
    ...
    def test_japanese_keywords_checked_first(self):
        ...
        import json
        from pathlib import Path
        sources_path = Path(__file__).resolve().parents[1] / 'src' / 'trendradar' / 'config' / 'sources.json'
