...
def test_parse_iso_cst_with_offset():
    ...
    from trendradar.runtime.time_utils import parse_iso_cst, CST
...
def test_lazy_caches_value():
    ...
    from trendradar.runtime.lazy import Lazy
    calls = [0]
    def factory():
        calls[0] += 1
        return "expensive"
...
def test_gen_run_id_with_slot():
    ...
    from trendradar.runtime.run_id import gen_run_id
...
def test_parse_run_id_roundtrip():
    ...
    from trendradar.runtime.run_id import gen_run_id, parse_run_id
...
def test_run_id_marker_contains_rid():
    ...
    from trendradar.runtime.run_id import run_id_marker
...
def test_run_id_contextvar():
    ...
    from trendradar.runtime.run_id import set_run_id_ctx, get_run_id_ctx, current_run_id
...
def test_exit_codes_values():
    """7 个退出码值固定, 防止误改。"""
    from trendradar.runtime.exit_codes import (
...
def test_get_data_dir_cached():
    ...
    from trendradar.runtime.file_utils import get_data_dir
    d1 = get_data_dir()
    d2 = get_data_dir()
...
def test_get_cache_dir_cached():
    from trendradar.runtime.file_utils import get_cache_dir
    assert get_cache_dir() is get_cache_dir()


def test_get_config_dir_cached():
    from trendradar.runtime.file_utils import get_config_dir
...
def test_write_read_compressed_roundtrip_json():
    ...
    """
    from trendradar.runtime.file_utils import write_compressed, read_compressed
    with tempfile.TemporaryDirectory() as tmp:
...
def test_common_re_exports_all_symbols():
    ...
    from trendradar.runtime import common
    expected = [
        "CST", "Lazy",
...
def test_common_lazy_is_same_as_lazy_module():
    """common.Lazy 应该就是 trendradar.runtime.lazy.Lazy。"""
    from trendradar.runtime.common import Lazy as CommonLazy
    from trendradar.runtime.lazy import Lazy as DirectLazy
    assert CommonLazy is DirectLazy


def test_common_cst_is_same_as_time_utils_cst():
    from trendradar.runtime.common import CST as CommonCST
    from trendradar.runtime.time_utils import CST as DirectCST
...
def test_find_curated_file_three_levels(tmp_path):
    ...
    """
    from trendradar.runtime import curated_paths
    curated_paths._data_dir_cache = None
    # 现在 init 走 tmp_path
    (tmp_path / "data").mkdir()
    curated_paths._data_dir_cache = tmp_path / "data"

    from trendradar.runtime.curated_paths import find_curated_file
...
def test_list_curated_files_respects_days(tmp_path):
    ...
    from trendradar.runtime.curated_paths import list_curated_files
