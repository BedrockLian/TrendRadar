...
SCRIPTS_DIR = str(PROJECT_PATH / 'src' / 'trendradar' / 'scripts')
TRENDRADAR_DIR = str(PROJECT_PATH / 'src' / 'trendradar')
...
class TestDedup:
    ...
    def test_dedup_empty(self):
        from trendradar.sources.fetch_feeds import _dedup
        assert _dedup({}) == []
...
class TestKwSets:
    def test_kw_sets_returns_tuple(self):
        from trendradar.sources.fetch_feeds import _kw_sets
...
class TestPreclassify:
    def test_preclassify_game_keyword(self):
        from trendradar.sources.fetch_feeds import _preclassify
