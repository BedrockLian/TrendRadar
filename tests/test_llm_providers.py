...
class TestBackwardCompatShim:
    """trendradar.intelligence.ai_translate._make_request() now returns str via LLMProvider."""
    ...
    async def test_make_request_returns_string(self, monkeypatch):
        ...
        monkeypatch.setattr(
            'trendradar.intelligence.llm_providers.OpenAIChatProvider.chat',
            fake_chat,
        )
        # Reload module to ensure shim is fresh
        sys.path.insert(0, str(Path(__file__).resolve().parents[1] / 'src'))
        from trendradar.intelligence.ai_translate import _make_request
        import aiohttp
