...
class TestCheckBannedPhrases:
    ...
    def test_detects_ai_language_model(self):
        from trendradar.reporting.sanity_check import check_banned_phrases
...
class TestCheckHtmlResidue:
    ...
    def test_detects_br_tag(self):
        from trendradar.reporting.sanity_check import check_html_residue
        text = "第一行<br>第二行"
...
class TestApplySensitiveFilter:
    ...
    def test_foreign_china_taiwan_filter(self):
        from trendradar.reporting.sanity_check import apply_sensitive_filter
        text = "台湾宣布新政策"
...
class TestCheckDeadLinks:
    ...
    def test_no_urls_returns_empty(self):
        from trendradar.reporting.sanity_check import check_dead_links
        text = "没有链接的文本"
        dead = check_dead_links(text)
        assert dead == []

    def test_extract_urls_from_markdown(self):
        from trendradar.reporting.sanity_check import _extract_urls
    ...
    @patch('trendradar.reporting.sanity_check.urllib.request.urlopen')
