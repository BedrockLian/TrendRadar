...
class TestCompare:
    ...
    def test_new_event_detected(self):
        from trendradar.reporting.track_events import compare
        today = [
...
class TestComputeHeat:
    ...
    def test_heat_includes_platform_count(self):
        from trendradar.reporting.track_events import compute_heat
        item = {
            '_heat': {'platform_count': 5},
            '_curator_scores': {'total': 10},
        }
...
class TestFindYesterdayMorning:
    ...
    def test_returns_empty_when_no_files(self, tmp_path, monkeypatch):
        from trendradar.reporting.track_events import find_yesterday_morning
        from trendradar.reporting import track_events
