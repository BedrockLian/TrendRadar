...
标准 Markdown 支持成对 **X** 渲染加粗。
flash sub-agent 常写出未闭合的 **（末尾多写当加粗没闭合）。
策略：就地补齐成对，让 Markdown 渲染为加粗。
- 末尾孤儿 X** → 补开 → **X** （X 被加粗）
- 段中粗体起点 X** Y → 第 1 个开 + 第 2 个闭 → 加粗 X** 之间的内容（实际是 "X"，第 2 个 ** 当闭）
- 堆俩 **** → 重叠合并 → **X**
- 已成对 **X** → 保留
"""
from trendradar.reporting.render_deep_analysis import format_analysis, clean
