...
def _has_source_keyword(keyword: str) -> bool:
    ...
    import json
    spath = PROJECT_ROOT / 'src' / 'trendradar' / 'config' / 'sources.json'
    if not spath.exists():
        return False
    try:
...
class TestLoadSourceLanguages:
    ...
    def test_returns_frozensets(self):
        from trendradar.intelligence.ai_translate import _load_source_languages
    ...
    def test_missing_sources_json_returns_empty(self):
        from trendradar.intelligence.ai_translate import _load_source_languages
        with patch('trendradar.intelligence.ai_translate._SOURCES_PATH', Path('/nonexistent/sources.json')):
            en, ja = _load_source_languages()
            assert en == frozenset()
            assert ja == frozenset()
...
class TestCircuitBreaker:
    ...
    def test_initial_state_not_broken(self):
        from trendradar.intelligence.ai_translate import circuit_broken, reset_circuit
        reset_circuit()
        assert circuit_broken() is False

    def test_broken_after_threshold(self):
        from trendradar.intelligence.ai_translate import circuit_broken, reset_circuit, CIRCUIT_BREAKER_THRESHOLD
...
class TestBatchTranslate:
    ...
    def test_successful_translation(self):
        ...
        async def _run():
            ...
            with patch('trendradar.intelligence.ai_translate._make_request', side_effect=fake_make_request):
...
class TestTranslateConfigConsistency:
    ...
    def test_all_sources_have_language(self):
        ...
        TR = Path(os.environ.get('TRENDRADAR_HOME', Path.cwd() / '.runtime'))
