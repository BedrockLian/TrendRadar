...
from trendradar.runtime.common import gen_run_id, parse_run_id, run_id_marker
...
class TestRecordFingerprints:
    ...
    def test_insert_includes_run_id(self, tmp_db, sample_curated, monkeypatch):
        ...
        (tmp_dir / f'curated_evening_{TEST_DATE}.json').write_text(
            json.dumps(curated_with_rid, ensure_ascii=False), encoding='utf-8')

        from trendradar.pipeline import record_fingerprints as rf
    ...
    def test_insert_new_items(self, tmp_db, sample_curated, monkeypatch):
        ...
        (tmp_dir / f'curated_evening_{TEST_DATE}.json').write_text(
            json.dumps(sample_curated, ensure_ascii=False), encoding='utf-8')
