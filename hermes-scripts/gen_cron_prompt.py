...
SCRIPTS_DIR = Path(__file__).resolve().parent.parent / 'trendradar' / 'scripts'
...
def generate_cron_prompt(steps: dict) -> str:
    ...
    lines.append('- `status: "ok"` → parse `fragments` array. For each fragment (except first if only header), use `send_message(target="wecom", message=fragment)` to deliver individually. This ensures WeCom 4KB limit is respected — auto-delivery of the full briefing is NOT sufficient.')
    ...
    lines.append("")
    lines.append("# 6. Split briefing into fragments and deliver via send_message")
    lines.append("```")
    lines.append("")
