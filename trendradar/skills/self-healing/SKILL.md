...
**诊断优先**: 任何异常先查 `references/traps.md`（30条陷阱全集）。本表仅收录 healing 专属修复。
...
| 维护脚本烟雾测试失败但不推送报警 | `trendradar_maintenance.py` 的 `runtests()` 旧用 `python3` 而非 python3.14t，且测试失败仅 `print(stderr)` → no_agent 只检查 exit code + stdout。**修复**: 改用 `$PYTHON` + 设 `PYTHONPATH` + 失败 `sys.exit(1)`。详见 `references/health-check-pitfalls.md #8` |
| 维护脚本备份列表含不存在文件 | 曾含 `push_log.json` 和 `preferences.json`（文件中已不存在），靠 `src.exists()` 静默跳过成为死代码。**修复**: 2026-05-24 已移除这两个条目 |
...
| 外媒 RSS 采集全部为空，国内源正常 | 米霍姆代理挂了。`systemctl --user status mihomo.service` → 如未运行则 `systemctl --user restart mihomo.service`。采集日志可见 `[FETCH] 38源（直连 8 + 代理 30）`，代理源应全空 |
| domain 连续多天覆盖率 <10% | 运行 `python3 scripts/blind_spot_audit.py --days 3 --json` 检查低覆盖板块。可能原因：该 domain 的 RSS 源全部失效 / timeline.yaml 未正确配时 / keywords 误过滤。按 `references/sources-management.md` 补充新源 |
...
| `references/migration-mechanism.md` | 迁移引擎架构：migrate()/repair_missing_tables() 详解 |
| `references/migration-rollback.md` | 迁移回滚约定：`-- down:` 注释 + down() 实现 |
| `references/health-check-design.md` | 体检设计：检查项表 + cron ID 表 |
| `references/health-check-pitfalls.md` | 维护陷阱：SKILL_DIR/导入方式/列表同步/日志/解释器/维护脚本一致性 |
