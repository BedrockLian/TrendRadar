...
### 4. `check_pipeline()` RSS 连通性：localhost 源假阳性
**现象**：健康检查随机抽 3 个 RSS 源做 socket 连通性测试，抽到 `localhost:1200`（本地通常未运行）的源时报 `RSS 源不可达`，导致每次运行结果不一致（有时通过有时报错）。

**修复**（2026-05-28）：
- 采样前过滤掉 `feed_url` 含 `localhost` 的源（本地非必需）
...
6. **源 RSS 源可用性** — 直接 `curl {feed_url}` 测试源响应。若 HTTP 200 有内容但 fetch_feeds 抓不到，可能是代理池耗尽或 aiohttp session 配置问题。单独测试 `_fetch_one()` 可确认。
