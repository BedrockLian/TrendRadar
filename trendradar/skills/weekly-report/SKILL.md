...
### 3. 输出
模板见 `references/weekly-format.md`。不重复日报格式（无🆕🔥标记）。
...
## 格式
简报格式和空行规范见 `references/render-format.md`。周报独立模板见 `references/weekly-format.md`。
...
| 文件 | 内容 |
|------|------|
| `references/weekly-format.md` | 周报格式模板 + 格式铁律 |
