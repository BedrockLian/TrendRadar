...
**Cron prompt 格式**: 自动生成。`trendradar/scripts/gen_cron_prompt.py` 从 `pipeline_orchestrator.py --list-steps` 产出 `trendradar/references/cron-prompt-generated.md`。news-secretary SKILL 引用此文件作为 SSOT，消除 cron prompt 与 SKILL 双份维护漂移。
...
│   ├── domains.py              # 领域常量 (DOMAINS, MAX_PER_DOMAIN, etc.)
...
`gen_cron_prompt.py` 2026-05-29 前长期处于语法错误状态（从未真正跑通过），修复后需手动 regenerate：`python3 scripts/gen_cron_prompt.py > references/cron-prompt-generated.md`。
## 两副本架构
...
| `references/CODE-AUDIT-AND-CONSOLIDATION.md` | **代码审计与精简操作手册**（1062行）— 100项发现 + 四阶段大修计划。执行大修前必读全文 |
...
`CODE-AUDIT-AND-CONSOLIDATION.md` 的第七章（`trendradar_health_check.py` 适配修改）容易被漏掉——它不在主 Phase 清单中，而是作为 Phase 1 的子项嵌入。第七章共 6 处修改：
