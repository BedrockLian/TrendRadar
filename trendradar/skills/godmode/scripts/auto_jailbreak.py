...
MODEL_STRATEGIES = {
    ...
    # DeepSeek V3/V4-Flash — keyword-based classifiers, parseltongue effective.
    # V4-Pro note: baseline already complies with gray-area queries (1 hedge).
    # Standard GODMODE prefill is the winner (score 300, 0 hedges).
    # System-prompt strategies need max_tokens >= 2048 or reasoning tokens
    # will exhaust the output budget (observed: 562 reasoning tokens on refusal_inv).
    # Boundary inversion fails completely on v4-pro.
    "deepseek": {
    ...
    # DeepSeek V4-Pro — baseline already complies, prefill-only wins.
    # System-prompt strategies need max_tokens >= 2048 due to reasoning token overhead.
    # OG GODMODE and refusal_inversion both work but are unnecessary — standard
    # prefill (GODMODE ENABLED) achieves the best results with zero hedges.
    "deepseek_v4_pro": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "deepseek" in model_lower:
        # V4-Pro has fundamentally different behavior from V4-Flash
        if "v4-pro" in model_lower or "v4_pro" in model_lower:
...
def _get_current_model() -> tuple:
    ...
    try:
        ...
        # Hermes config uses 'default' not 'name'
        model_name = model_cfg.get("name") or model_cfg.get("default", "")
        base_url = model_cfg.get("base_url", "https://openrouter.ai/api/v1")
        # Strip OpenRouter provider prefix for direct API calls
        if base_url and "openrouter" not in base_url and "/" in model_name:
...
def _get_api_key(base_url: str = None) -> str:
    """Get the appropriate API key. Supports all common Hermes providers."""
    if not base_url:
        # Try common providers
        for var in ["OPENROUTER_API_KEY", "DEEPSEEK_API_KEY", "ANTHROPIC_API_KEY",
                     "OPENAI_API_KEY", "GEMINI_API_KEY", "MINIMAX_CN_API_KEY",
                     "GROK_API_KEY", "TOGETHER_API_KEY"]:
            val = os.getenv(var, "")
            if val:
                return val
        return ""
    ...
    if "deepseek" in bl:
        return os.getenv("DEEPSEEK_API_KEY", os.getenv("OPENROUTER_API_KEY", ""))
    if "anthropic" in bl:
    ...
    if "grok" in bl or "xai" in bl:
        return os.getenv("GROK_API_KEY", "")
    if "together" in bl:
        return os.getenv("TOGETHER_API_KEY", "")
    # Fallback: try any set API key
    for var in ["OPENROUTER_API_KEY", "DEEPSEEK_API_KEY", "ANTHROPIC_API_KEY",
