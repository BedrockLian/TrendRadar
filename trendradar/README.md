...
TrendRadar 是一个轻量级新闻聚合与智能推送系统。它从 39+ 个 RSS 源和博客异步抓取内容，通过 AC 自动机分类 + AI 评分后，按早/午/晚三个时段推送 Markdown 简报至企业微信。
...
```
┌─────────────┐    ┌──────────────┐    ┌───────────────┐    ┌──────────────┐
│  RSS Fetch  │ →  │  AC Classify │ →  │  AI Rendering │ →  │  WeCom Push  │
│  39 sources │    │  5 domains   │    │  5-way async  │    │  fragmented   │
│  async/39   │    │  frozenset   │    │  ~9s total    │    │  1.5s interval│
└─────────────┘    └──────────────┘    └───────────────┘    └──────────────┘
```
...
- **多源异步抓取** — 39 RSS/博客源，aiohttp 异步并发，两级连接池（RSSHub + 外网直连）
- **AC 自动机分类** — 505 关键词 × 6 域，frozenset O(1) 查找，比线性匹配快 4.4×
- **AI 渲染** — 各板块独立调用 DeepSeek Flash API，5 路并行，~9s 完成全简报
...
| `config/translate.yaml` | 14 个英文源翻译配置 |
| `config/ai_interests.yaml` | 兴趣偏好（LLM Agent 可自动管理） |
| `data/sources.json` | 39 个 RSS/博客源清单 |
...
│   ├── fetch_feeds.py       # 39 源异步抓取
│   ├── curate_and_push.py   # AC 分类 + 评分精选
│   ├── render_briefing.py   # AI 渲染（5 路并行 API）
