...
> 简报渲染已由 `render_markdown.py` 完全接管（纯脚本，~0s，零 token 成本）。
> 格式硬编码，永远一致。本文档是格式的唯一权威来源。
...
### 1. 板块标题硬编码
脚本用 `DOMAIN_LABELS` 中的硬编码值生成板块标题，emoji 与标签绑定：
- `📰 头条` `🌏 外媒看华` `💻 科技` `📊 经济民生` `🎮 游戏`
...
如果 `render_markdown.py` 失败（返回码非零或 stdout 为空），检查 curated JSON 是否存在。
