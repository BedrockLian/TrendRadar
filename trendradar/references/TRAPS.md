...
<!-- 本文件为陷阱全集。各陷阱来源已归档至 git history。 -->
...
## 46. 性能瓶颈：TCP 连接池耗尽
**症状**：RSS 源 aiohttp 超时但 curl 正常。**修复**（2026-05-21）：将 `TCPConnector(limit)` 设为不低于 `Semaphore` 值+20%余量。当前已统一为单 `EXTERNAL_CONCURRENT=20` + `TCPConnector(limit=20)`，**已修复**。
...
## 49. _load_interests() 滑窗停用词陷阱 [interest_loader.py]
`_load_interests()` 用 2-3 字滑窗从排除短语提取关键词。通用词（新闻/游戏/体育/行业/重大/娱乐/明星）可能误入排除集 → 标题含这些词的条目全部 score=0。**修复**：STOP_WORDS 集合必须包含上述通用词。新增排除短语时检查 2-3 字拆解产物。**诊断**：`from trendradar.scripts.interest_loader import load_interests; pos, neg = load_interests(); assert '新闻' not in neg`
