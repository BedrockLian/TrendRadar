...
> **注意**：`allow-lan: true` 和 `bind-address: "0.0.0.0"` 是必需的——局域网内其他设备需要访问 Mihomo。
...
│   └── delivery_watchdog.py           # 推送看门狗 + auto-redeliver（早/午/晚）
├── .gitignore
├── LICENSE
└── README.md
```
...
```bash
cd ~/TrendRadar/trendradar
PYTHONPATH=$HOME/.hermes python3.14t -c "
...
> 必须设置 `PYTHONPATH` 为其**父目录**（即 `$HOME/.hermes`），而不是项目根目录本身。
...
| 问题 | 原因 | 解决 |
|------|------|------|
| `ModuleNotFoundError: trendradar` | PYTHONPATH 缺失 | `export PYTHONPATH=$HOME/.hermes` |
...
cp -r ~/TrendRadar/trendradar/skills/report-generator ~/.hermes/skills/trendradar/
```
...
# system-config           系统配置速查
# report-generator        周报/月报深度研判 + 推送质量优化
```

## 8. 注册定时任务

TrendRadar 的完整功能依赖 6 个 cron 定时任务，构成三层推送体系：
...
| ⚙️ 运维 | 体检/维护/看门狗 | 每日/按需 | no_agent | 健康检查/备份/告警 |
...
hermes cron create \
  ...
  --schedule "30 9 * * 1" \
  --skills multi-search-engine,deep-research-cli,report-generator \
...
| import 错误 | `export PYTHONPATH=$HOME/.hermes` |
